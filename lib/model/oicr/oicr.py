...
from model.oicr.CAMP import CAM_DROPBLOCK
...
class _OICR(nn.Module):
    ...
    def __init__(self, classes, class_agnostic, tb=None):
        ...
        self.tb = tb
      #******************************************#
        self.CA=CAM_DROPBLOCK(inchannel=512)
    ...
    def forward(self, im_data, rois, labels=None, num_boxes=None):
        ...
        if self.training : # for multi-GPU
            ...
            num_boxes = num_rois
        base_feat = self.OICR_base(im_data)#[1,512,60,65]
        rois = Variable(rois)
        # ******************************************#
        if self.training:
            self.mask,self.out_cam=self.CA(base_feat.detach(),0.3)#mask[1,60,65]
            #print(self.mask.sum())
            base_feat=base_feat*self.mask
        # ******************************************#
        ...
        self.midn_prob = self.midn_prob0 * self.midn_prob1 #[1,1420,20]

        if self.training:
            labels = labels.data#（1。20）
            ...
            #/**********************************************************************/#
            loss_cam = multi_class_cross_entropy_loss(self.out_cam, labels)
            ...
            return init_rois, loss_midn.view(1), loss_oicr.view(1), loss_oicr1.view(1), loss_oicr2.view(1), ret_prob,loss_cam.view(1)
        else :
